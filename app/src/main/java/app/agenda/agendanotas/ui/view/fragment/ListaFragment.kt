package app.agenda.agendanotas.ui.view.fragment

import android.os.Bundle
import androidx.fragment.app.Fragment
import android.view.LayoutInflater
import android.view.View
import android.view.ViewGroup
import androidx.lifecycle.Observer
import androidx.lifecycle.ViewModelProvider
import androidx.recyclerview.widget.LinearLayoutManager
import app.agenda.agendanotas.R
import app.agenda.agendanotas.databinding.FragmentListaBinding
import app.agenda.agendanotas.ui.adapter.NoteAdapter
import app.agenda.agendanotas.ui.viewmodel.NotaViewModel


class ListaFragment : Fragment() {
   private lateinit var binding: FragmentListaBinding
   private lateinit var model:NotaViewModel
   private lateinit var cadapter:NoteAdapter

    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding= FragmentListaBinding.inflate(inflater,container,false)
        model=ViewModelProvider(requireActivity()).get(NotaViewModel::class.java)
        initRecycler()
        getAllNotes()
        return binding.root
    }
    private fun initRecycler(){
        cadapter= NoteAdapter()
        binding.recycler.apply {
            layoutManager=LinearLayoutManager(requireContext())
            adapter=cadapter
            setHasFixedSize(true)
        }
    }
    private fun getAllNotes(){
        model.getAllNotes().observe(viewLifecycleOwner, Observer {
            cadapter.differ.submitList(it)
        })
    }


}